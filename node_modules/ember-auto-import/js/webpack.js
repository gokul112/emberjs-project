...
class WebpackBundler extends broccoli_plugin_1.default {
    ...
    babelRule(stagingDir, filter, babelConfig) {
        let shouldTranspile = shared_internals_1.babelFilter(this.skipBabel(), this.opts.rootPackage.root);
        return {
            test: (filename) => {
                ...
                return (path_1.dirname(filename) !== stagingDir &&
    ...
    get externalsHandler() {
        ...
        return (params, callback) => {
            ...
            if (!name) {
                if (!path_1.isAbsolute(request) && pkg.root === this.opts.rootPackage.root) {
                    let appRelativeContext = path_1.relative(path_1.resolve(this.opts.rootPackage.root, 'app'), context);
            ...
            if (pkg.isV2Addon() && ((_a = pkg.meta.externals) === null || _a === void 0 ? void 0 : _a.includes(name))) {
                this.externalizedByUs.add(request);
                return callback(undefined, 'commonjs ' + request);
            }
            try {
    ...
    *withResolvableExtensions(importSpecifier) {
    ...
    importMatchesAppImports(relativeImportSpecifier) {
        for (let candidate of this.withResolvableExtensions(relativeImportSpecifier)) {
            if (this.opts.rootPackage.allowAppImports.some((pattern) => minimatch_1.default(candidate, pattern))) {
                return true;
            }
        }
        return false;
    ...
    matchesAppImports(pkg, requestingFile) {
        ...
        return this.opts.rootPackage.allowAppImports.some((pattern) => minimatch_1.default(path_1.relative(path_1.join(pkg.root, 'app'), requestingFile), pattern));
    }
    build() {
    ...
    addDiscoveredExternals(build) {
        for (let assetFiles of build.entrypoints.values()) {
            for (let assetFile of assetFiles) {
                let inputSrc = fs_1.readFileSync(path_1.resolve(this.outputPath, assetFile), 'utf8');
                let outputSrc = inputSrc.replace(/EAI_DISCOVERED_EXTERNALS\(['"]([^'"]+)['"]\)/g, (_substr, matched) => {
                    let deps = build
                        .externalDepsFor(matched)
                        .filter((dep) => this.externalizedByUs.has(dep));
                    return '[' + deps.map((d) => `'${d}'`).join(',') + ']';
                });
                fs_1.writeFileSync(path_1.resolve(this.outputPath, assetFile), outputSrc, 'utf8');
            }
        }
    }
